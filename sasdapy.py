    ...
    while True:
        ...
        if user_choice == '1':
            ...
            try:
                ...
                c = conn.cursor()
                c.execute('CREATE TABLE IF NOT EXISTS PassDb([FName] TEXT PRIMARY KEY,[LName] TEXT, [Birth] TEXT, [Pass] TEXT, [LOGName] TEXT)')
                add = ('''INSERT INTO PassDb (Fname, Lname, Birth, Pass, LOGName) VALUES (?,?,?,?,?)''')
                c.execute(add, (fname, lname, birthdate, password, "result.txt"))
                conn.commit()
